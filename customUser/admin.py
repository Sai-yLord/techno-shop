...
class UserChangeForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('fullname' , 'username', 'email', 'password', 'is_active', 'is_superuser')
...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = (
        (None, {'fields': (
            'fullname', 'username', 'email', 'password', 'is_active', 'is_staff', 'is_superuser')}),
    )
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': (
                'fullname', 'username', 'email', 'password1', 'password2', 'is_active', 'is_staff',
                'is_superuser'),
        }),
    )
    search_fields = ('email',)
